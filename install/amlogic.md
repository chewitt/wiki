...
The "box" image supports hardware like Set-Top "Box" (STB) devices which ship with Android (and Amlogic u-boot) installed on the internal eMMC storage. It is common for box vendors to tweak RAM timings and make other boot-time customisations so it is best to run LibreELEC from an SD card and leave the vendor u-boot intact. To use a box image you trigger "update" mode in the vendor u-boot. This causes u-boot to look for some standard filesnames which we have tweaked to load the LibreELEC `KERNEL` and `SYSTEM` files to boot the device.
